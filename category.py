...
def category_mkdir(category_name):
    Path(os.path.join(MARKDOWN_FILE_PATH,
                      category_name)).mkdir(parents=True, exist_ok=True)
...
def load_categories_from_tistory():
    ...
    category_from_tistory = requests.get(category_url,
                                         params=category_params).json()
