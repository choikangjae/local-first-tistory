...
def auth():
    ...
    if BLOG_NAME is None:
        ...
        set_key(key_to_set="REDIRECT_URI", value_to_set=f"{blog_name}.tistory.com", dotenv_path=dotenv_path)
        print("블로그 이름 저장 완료")

    if APP_ID is None:
        print("https://www.tistory.com/guide/api/manage/register 에서 App ID와 Secret Key를 발급받아주세요.")
    ...
    if SECRET_KEY is None:
        ...
        set_key(key_to_set="SECRET_KEY", value_to_set=secret_key, dotenv_path=dotenv_path)
    ...
    if AUTHORIZATION_CODE is None:
        ...
        code = input("""code를 입력해주세요. code는 다음과 같은 형태이며 리다이렉트 된 주소를 통해 확인할 수 있습니다.
    https://www.tistory.com/oauth/your_blog_name.tistory.com?code={{{code}}}&state=
: """)
        set_key(key_to_set="AUTHORIZATION_CODE", value_to_set=code, dotenv_path=dotenv_path)
        print("code 저장 완료")
